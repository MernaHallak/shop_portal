...
---------------------------------
## الفرق بين refresh_token و access_token:
...
access_token: الباك يتحقق منه ليعرف:  مين المستخدم؟ هل مسجل دخول؟ هل عنده الصلاحية المطلوبة؟ 

refresh_token : الـ refresh token بالنسبة للباك هو:  دليل إن عندك جلسة طويلة الأمد مسموح إلها تطلب access token جديد. backend يتحقق:هل refresh token صحيح؟ هل منتهي؟   

Access token
= مفتاح أستخدمه كل يوم للدخول للـ APIs

Refresh token
= مفتاح احتياطي أستخدمه فقط للحصول على Access token جديد

ولهذا منطقي جدًا نخلي:
refresh_token path = "/api/auth/refresh"
حتى ما ينبعت إلا لما نحتاج فعلًا نجدد الجلسة.
